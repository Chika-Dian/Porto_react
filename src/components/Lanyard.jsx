...
const CARD_GLB_PATH = '/assets/card.glb';
const LANYARD_PNG_PATH = '/assets/lanyard.png';

extend({ MeshLineGeometry, MeshLineMaterial });

export default function Lanyard({ transparent = true }) {
 
    const CUSTOM_CAMERA_POSITION = [0, 1, 8]; 
    const CUSTOM_FOV = 50; 
    const CUSTOM_GRAVITY = [0, -25, 0]; 

    return (
        
        <div className="relative z-0 w-full h-full flex justify-center items-center"> 
            <Canvas
                camera={{ position: CUSTOM_CAMERA_POSITION, fov: CUSTOM_FOV }}
                gl={{ alpha: transparent }}
                onCreated={({ gl }) => gl.setClearColor(new THREE.Color(0x000000), transparent ? 0 : 1)}
            >
                <ambientLight intensity={Math.PI} />
                <Physics gravity={CUSTOM_GRAVITY} timeStep={1 / 60}> 
                    <Band />
                </Physics>
...
function Band({ maxSpeed = 50, minSpeed = 0 }) {
    ...
    return (
        <>
            {/* Posisi gantung diturunkan ke Y=5 agar tali terlihat utuh dari atas */}
